...
def get_task_keyboard(
    ...
    builder.append([
        InlineKeyboardButton(
            text="💡 Подсказка 1",
            callback_data=f"th:{date_str}:h1"
        ),
        InlineKeyboardButton(
            text="💡 Подсказка 2",
            callback_data=f"th:{date_str}:h2"
        )
    ])
...
async def send_daily_task(
    ...
    # Награда за просмотр сегодняшней задачи

    if date_str == get_yerevan_date():

        uid_str = str(user_id)

        if "opened_tasks" not in DATABASE["users"].get(
            uid_str,
            {}
        ):
        ...
        if date_str not in DATABASE["users"][uid_str]["opened_tasks"]:
            ...
            DATABASE["users"][uid_str]["opened_tasks"].append(
                date_str
            )
...
async def task_hint_handler(
    ...
    if not val:
        val = (
            "Пусто. Админ не добавил этот пункт 😔"
        )
...
async def show_favorites(target):
    ...
    if not favs:
        ...
        text = (
            "❤️ **Избранное**\n\n"
            "Тут пока пусто. Добавляй файлы "
            "с помощью кнопки под ними!"
        )
...
async def view_file(
    ...
    if not f:
        ...
        return await callback.answer(
            "❌ Файл больше не доступен или был удален.",
            show_alert=True
        )
...
# глобальный поиск работает ТОЛЬКО когда
# пользователь не находится ни в одном FSM-состоянии.
#
# Поэтому текст задачи больше не будет
# перехватываться этим обработчиком.
...
# Этот обработчик нужен именно для кнопки
# "🔎 Поиск", которая использует
# switch_inline_query_current_chat.
...
async def inline_search_handler(
    ...
    # Если пользователь только открыл поиск
    if not query:
...
async def adm_task_photo(
    ...
    await state.set_state(
        TaskOfDayAdmin.waiting_for_caption
    )

    await message.answer(
        "Отправь текст/условие задачи "
...
async def adm_task_cap(
    ...
    await state.set_state(
        TaskOfDayAdmin.waiting_for_hint1
    )
...
async def adm_task_h1(
    ...
    await state.set_state(
        TaskOfDayAdmin.waiting_for_hint2
    )
...
async def fe_menu(
    ...
    await callback.message.edit_reply_markup(
        reply_markup=get_file_edit_keyboard(uid)
    )
...
async def fe_diff_menu(
    ...
    await callback.message.edit_reply_markup(
        reply_markup=get_difficulty_keyboard(uid)
    )
