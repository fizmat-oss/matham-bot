...
def award_points(user_id: int, points: int):
    uid_str = str(user_id)
    ...
    for cat_data in data["categories"].values():
        for f in cat_data.get("files", []):
            if "file_unique_id" not in f:
                f["file_unique_id"] = (
                    f.get("file_id", str(uuid.uuid4()))[-15:]
                    + uuid.uuid4().hex[:5]
                )
...
class AdminEditSubmission(StatesGroup):
    waiting_for_title = State()
...
def get_catalog_keyboard():
    ...
    for cat_key, cat_data in DATABASE["categories"].items():
        builder.append([
            InlineKeyboardButton(text=cat_data["title"], callback_data=f"cat:{cat_key}")
        ])
    builder.append([
...
def build_admin_categories_kb(selected: set):
    ...
    for cat_key, cat_data in DATABASE["categories"].items():
        ...
        builder.append([
            InlineKeyboardButton(text=f"{mark} {cat_data['title']}", callback_data=f"a_toggle:{cat_key}")
        ])
...
def build_user_categories_kb(selected: set):
    ...
    for cat_key, cat_data in DATABASE["categories"].items():
        mark = "☑️" if cat_key in selected else "▫️"
        builder.append([
            InlineKeyboardButton(text=f"{mark} {cat_data['title']}", callback_data=f"usub_toggle:{cat_key}")
        ])
...
def build_submission_categories_kb(sub_id: str):
    ...
    for cat_key, cat_data in DATABASE["categories"].items():
        mark = "☑️" if cat_key in selected else "▫️"
        builder.append([
            InlineKeyboardButton(text=f"{mark} {cat_data['title']}", callback_data=f"subcat_toggle:{sub_id}:{cat_key}")
        ])
...
def get_file_edit_categories_kb(uid: str, selected: set):
    ...
    for cat_key, cat_data in DATABASE["categories"].items():
        mark = "☑️" if cat_key in selected else "▫️"
        builder.append([
            InlineKeyboardButton(text=f"{mark} {cat_data['title']}", callback_data=f"fec_t:{uid}:{cat_key}")
        ])
...
async def cmd_start(message: types.Message, state: FSMContext):
    await state.clear()
    await track_user_activity(message.from_user.id, message.from_user.username)
...
async def cmd_menu(message: types.Message, state: FSMContext):
    await state.clear()
    await track_user_activity(message.from_user.id)
...
async def process_back_to_main(callback: types.CallbackQuery):
    await track_user_activity(callback.from_user.id)
...
def get_task_keyboard(date_str: str, admin_view: bool = False):
    ...
    builder.append([InlineKeyboardButton(text="✅ Решение", callback_data=f"th:{date_str}:sol")])
    builder.append([
...
async def task_vote_handler(callback: types.CallbackQuery):
    ...
    if uid_str not in votes:
        award_points(callback.from_user.id, 2)
...
async def task_hint_handler(callback: types.CallbackQuery):
    ...
    if hint_type == "h1": val = task.get("hint1", "")
    elif hint_type == "h2": val = task.get("hint2", "")
    elif hint_type == "sol": val = task.get("solution", "")
    else: val = ""

    if not val: val = "Пусто. Админ не добавил этот пункт 😔"
...
async def mustread_main(callback: types.CallbackQuery):
    ...
    if not files:
        return await callback.message.edit_text(
            "⭐ **MUST-READ**\n\nПока пусто.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Меню", callback_data="menu:main")]])
        )
...
async def toggle_fav(callback: types.CallbackQuery):
    ...
    if uid in favs:
        ...
        favs.append(uid)
        await callback.answer("❤️ Добавлено из избранного")
...
async def show_rating(target):
    ...
    text += f"\n🔥 **Твой streak:** {my_user.get('streak', 0)} дней подряд"
    text += f"\n🎯 **Твои очки:** {my_user.get('score', 0)}"
...
async def process_random(target, diff: str):
    ...
    award_points(target.from_user.id, 1)
...
async def inline_search_handler(inline_query: types.InlineQuery):
    ...
    for cat_key, cat_data in DATABASE["categories"].items():
        for f in cat_data.get("files", []):
            ...
            haystack = (
                f"{cat_data['title']} {f.get('caption', '')} "
                + " ".join(f.get("tags", []))
            ...
            if all(word in haystack for word in words):
                ...
                results.append(
                    types.InlineQueryResultCachedDocument(
                        id=uid,
                        title=f.get("caption", "Без названия"),
                        description=cat_data["title"],
                        document_file_id=f["file_id"],
                        caption=f"📄 {f.get('caption', 'Без названия')}\n📌 {cat_data['title']}"
                    )
                )
...
async def admin_panel(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔗 Ссылки", callback_data="links:main")],
...
async def admin_broadcast_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("📢 Отправь сообщение для рассылки всем пользователям бота (текст, фото с подписью и т.д.).\n\nДля отмены напиши 'отмена'.")
    await callback.answer()
...
async def broadcast_confirm(callback: types.CallbackQuery, state: FSMContext):
    ...
    success, errors = 0, 0
    for uid_str in DATABASE["users"].keys():
        try:
...
async def adm_task_date(message: types.Message, state: FSMContext):
    ...
    if date_str.lower() == "сегодня": date_str = get_yerevan_date()

    try: datetime.strptime(date_str, "%Y-%m-%d")
    except ValueError: return await message.answer("Неверный формат. Используй YYYY-MM-DD или 'сегодня'.")
...
async def fe_menu(callback: types.CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id): return
    uid = callback.data.split(":")[1]
    f = get_file_by_uid(uid)
...
async def admin_doc_received(message: types.Message, state: FSMContext):
    ...
    if curr_state == EditFile.waiting_for_document.state:
        return
...
async def sub_approve(callback: types.CallbackQuery):
    ...
    try:
        await bot.send_message(sub["user_id"], f"✅ Твой файл «{sub['title']}» добавлен! Спасибо 🙌 (+15 очков)")
    except Exception: pass
...
async def sub_editcat(callback: types.CallbackQuery):
    if not is_admin(callback.from_user.id): return
    sub_id = callback.data.split(":")[1]
    if PENDING_SUBMISSIONS.get(sub_id, {}).get("status") != "pending": return
...
async def sub_edittitle(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminEditSubmission.waiting_for_title)
    await state.update_data(sub_id=sub_id, msg_id=callback.message.message_id)
    await callback.message.answer("Введи новое название для этого предложенного файла:")
    await callback.answer()

@dp.message(AdminEditSubmission.waiting_for_title, F.text)
async def sub_save_title(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
    data = await state.get_data()
    sub_id = data.get("sub_id")

    if sub_id in PENDING_SUBMISSIONS:
        PENDING_SUBMISSIONS[sub_id]["title"] = message.text.strip()
        sub = PENDING_SUBMISSIONS[sub_id]
        cats_text = ", ".join(DATABASE["categories"][c]["title"] for c in sub["categories"]) if sub["categories"] else "Не указано"
        caption = f"📥 **Новый файл**\n👤 От: @{sub['username']}\n📄 {sub['title']}\n📁 {cats_text}"
    ...
    await state.clear()
    await message.answer("✅ Название предложения обновлено!")
